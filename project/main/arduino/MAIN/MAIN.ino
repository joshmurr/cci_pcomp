...
uint16_t packetSize;    // expectend DMP packet size (default is 42 bytes)
...
void fadeEyes() {
  for (int i = dim_eyes; i < bright_eyes; i++) {
    analogWrite(LEFT_EYE, i);
    analogWrite(RIGHT_EYE, i);
    delay(50);
  }
  delay(150);
  for (int i = bright_eyes; i < dim_eyes; i++) {
    analogWrite(LEFT_EYE, i);
    analogWrite(RIGHT_EYE, i);
    delay(50);
  }
